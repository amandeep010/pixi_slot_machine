...
const InitialGameSetup = async () => {
  ...
  const manifest = {
    bundles: [
      ...
      {
        name: "frame",
        assets: [{ alias: "frame", src: "./assets/frame.png" }],
      },
      {
          name: "buttons",
          assets: [
  ...
  const frame = new Sprite(frametexture.wheel);
  ...
  spinWheel.on("click", async () => await RotateSlots(spinWheel, spriteSheet))
